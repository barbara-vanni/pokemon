    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if get_combat() == 2:
                if suite_button.draw(screen) == True:
                    ...
                    if mort == True:
                        ...
                        if level_up == True:
                            pokedex.change_statistics(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_xp(), combat_begin.get_pokemon1().get_xp_max(), "save")
                            pokedex.change_stat_pv(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_pv(), "save")
                            set_combat(4)
                        print(mort)
                        if combat_begin.get_pokemon2() != combat_begin.get_pokemon_player():
                            level_up = combat_begin.gain_xp()
                            if level_up == True:
                                pokedex.change_statistics(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_xp(), combat_begin.get_pokemon1().get_xp_max(), "save")
                                pokedex.change_stat_pv(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_pv(), "save")
                                ...
                                pokedex.change_stat_pv(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_pv(), "save")
                                pokedex.change_stat_xp(combat_begin.get_pokemon1().get_name(), "save")
                                set_combat(3)
                        else:
                            pokedex.change_stat_pv(combat_begin.get_pokemon1().get_name(), combat_begin.get_pokemon1().get_pv(), "save")
                            pokedex.change_stat_xp(combat_begin.get_pokemon1().get_name(), "save")
                            set_combat(3)
                            pokedex.level_down(combat_begin.get_pokemon2().get_name())
                            print(combat_begin.get_pokemon2().get_name())
                            set_combat(5)
                    else:
                        if turn_number == 1:
                            combat_begin.end_attack()
            ...
            if get_combat() == 3 or get_combat() == 4:
                if get_combat() == 3 or get_combat() == 4 or get_combat() == 5:
                    if suite_button.draw(screen) == True:
